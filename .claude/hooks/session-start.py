...
"""
        ...
        # Check both sessions/tasks and root tasks directory
        tasks_dir = sessions_dir / 'tasks'
        root_tasks_dir = PROJECT_ROOT / 'tasks'
        task_files = []

        # Collect from sessions/tasks if it exists
        if tasks_dir.exists():
            task_files.extend(sorted([f for f in tasks_dir.glob('*.md') if f.name != 'TEMPLATE.md']))

        # Collect from root tasks directory if it exists
        if root_tasks_dir.exists():
            task_files.extend(sorted([f for f in root_tasks_dir.glob('*.md') if f.name != 'TEMPLATE.md']))
