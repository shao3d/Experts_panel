...
# Track which type of feedback we're giving
daic_reminder = False
cd_feedback = False
...
# Exit strategy to avoid concurrency issues:
# - DAIC reminders (parallel Edit/Write): exit 0 (silent, no feedback to Claude)
# - cd commands (single Bash): exit 2 (send feedback to Claude)
if cd_feedback:
    sys.exit(2)  # Send cd feedback to Claude
elif daic_reminder:
    sys.exit(0)  # Silent exit for parallel operations
else:
    sys.exit(0)