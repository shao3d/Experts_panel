...
- **🔥 НОВАЯ ЛОГИКА**: Проверка комментариев для ВСЕХ новых постов + последние SYNC_DEPTH постов (настраивается в .env)
...
- **Глубина**: SYNC_DEPTH из .env (по умолчанию 10, можно переопределить через --depth N)
