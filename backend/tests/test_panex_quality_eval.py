...
def test_quality_scenarios_fixture_has_realistic_panex_cases():
    ...
    assert {
        ...
        "and23_expand_named_expert",
        "and23_expand_claim",
        "and23_expand_comments_focus",
    ...
    for scenario in scenarios:
        ...
        assert scenario["response_mode"] in {
            "expert_digest",
            "source_expand",
            "clarification",
            "boundary",
...
def test_and23_selector_answers_pass_bdd_guardrails():
    cases = {
        "and23_expand_named_expert": _good_and23_named_expert_expansion(),
        "and23_expand_claim": _good_and23_claim_expansion(),
        "and23_expand_comments_focus": _good_and23_comments_expansion(),
        "and23_expand_weak_sources": _good_and23_weak_sources_expansion(),
        "and23_ambiguous_selector_clarifies": _good_and23_ambiguous_clarification(),
        "and23_no_previous_digest_boundary": _good_and23_no_previous_digest_boundary(),
    }
...
def test_and23_selector_bad_answers_fail_when_they_guess_or_rerun():
    cases = {
        "and23_expand_named_expert": (
            "query_sent: Панэкс, раскрой по Рефату. experts_sent: refat. "
            "response_mode: expert_digest. target: Fly.io. warnings: none. "
            "Запущу новый digest и новый поиск по всем источникам: refat:101."
        ),
        "and23_ambiguous_selector_clarifies": (
            "Request passport: source_keys_sent: refat:101. mode: source_expand. "
            "target: Fly.io. warnings: none. Я сам выбрал этот вывод без уточнения."
        ),
        "and23_no_previous_digest_boundary": (
            "Request passport: source_keys_sent: refat:101. mode: source_expand. "
            "target: Fly.io. warnings: none. Раскрыл refat:101 из памяти."
        ),
    }
...
def _good_and23_named_expert_expansion() -> str:
    return """
**Request passport**
...
- refat:101: что в источнике важно - это раскрытие источника из предыдущего
  digest по Рефату, а не новый поиск. Сам источник даёт практический сигнал:
  автор описывает, почему разделение контекста и роли помогает только когда
  задача действительно распадается на самостоятельные куски работы.
- Комментарии под источником добавляют нюанс, но не превращают вывод в факт:
  часть обсуждения скорее уточняет ограничения и показывает, где source-backed
  signal слабее.
- Это поддерживает прежний вывод, но не меняет его: по этим источникам видно
  осторожное "может быть полезно", а не доказательство универсального правила.
"""


def _good_and23_claim_expansion() -> str:
    return """
**Request passport**
...
- refat:101: этот вывод про plan-first дисциплину основан на источнике, где
  автор связывает качество работы агента с предварительным разбиением задачи,
  явным scope и проверяемым результатом. Это supporting_sources для тезиса,
  а не самостоятельная истина.
- akimov:201: источник добавляет управленческий нюанс: план полезен, когда
  снижает coordination overhead и помогает понять, где нужен отдельный агент.
- Комментарии скорее уточняют ограничения: plan-first не должен превращаться
  в тяжёлый процесс для маленькой правки. Поэтому прежний digest поддержан,
  но источник оставляет caveat про размер задачи и стоимость координации.
"""


def _good_and23_comments_expansion() -> str:
    return """
**Request passport**
...
- doronin:101: источник выбран как самый спорный/ограниченный из предыдущего
  digest, поэтому фокус здесь на комментариях, а не на пересборке позиции
  эксперта.
- Комментарии добавляют скорее нюанс, чем новый сильный сигнал: часть ветки
  спорит о применимости тезиса, часть выглядит как шум/noise и не должна
  усиливать вывод.
- Ограничение: по одному источнику нельзя делать общий вывод. Он помогает
  увидеть слабое место прежнего ответа и то, где комментарии поддерживают
  осторожную трактовку, но не доказывают её.
"""


def _good_and23_weak_sources_expansion() -> str:
    return """
**Request passport**
...
- refat:101: слабое место источника в том, что он даёт практический сигнал
  про разделение ролей, но не показывает прямой критерий для всех задач.
  Это полезный source-backed signal, не факт и не доказательство.
- akimov:201: источник слабее как доказательство, потому что больше говорит
  о стоимости координации и риске лишнего процесса. Он ограничивает вывод,
  а не усиливает его.
- Комментарии помогают проверить нюанс, но часть обсуждения недостаточно
  предметная. Поэтому слабые источники лучше читать как ограничения прежнего
  ответа: они показывают, где нужен следующий вопрос или ручная проверка.
"""


def _good_and23_ambiguous_clarification() -> str:
    return """
Уточню одним сообщением, прежде чем дергать source expansion: какой именно
вывод раскрыть - тезис по Refat, тезис по Akimov или конкретный source handle
из предыдущего digest? Сейчас формулировка "этот вывод" может относиться к
нескольким сигналам, поэтому я не буду гадать и выбирать источник сам.

Ответь коротко, например: "раскрой вывод Рефата про plan-first" или "раскрой
источник akimov:201". После этого можно раскрыть ровно выбранные источники.
"""


def _good_and23_no_previous_digest_boundary() -> str:
    return """
Не буду гадать по Refat: у меня нет previous digest / предыдущего digest с
source handles в текущем контексте, поэтому я не могу честно выбрать источник
для раскрытия и не буду придумывать refat:* из памяти.

Сначала нужен основной вопрос к Панэксу, например: "Панэкс, спроси Рефата:
когда subagents помогают?". После такого ответа появятся source handles, и
тогда можно сказать простым языком: "раскрой по Рефату" или "покажи самый
сильный источник".
"""
