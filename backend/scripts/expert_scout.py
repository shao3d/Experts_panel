...
(sanitize_fts5_query + sqlite-vec KNN + RRF) in a standalone form so the
scout can run without the full FastAPI stack. Consolidate later if the
scout outlives the experiment.
"""
...
def _fts_search(
    ...
    return [(int(pid), str(snip)) for pid, snip in rows], None
...
def _vector_search(
    ...
    for expert_id in expert_ids:
        ...
        try:
            ...
            return [], f"vector_error: {exc}"
        for post_id, distance in rows:
            scored[int(post_id)] = float(distance)
    ordered = sorted(scored.items(), key=lambda item: item[1])
    return [post_id for post_id, _ in ordered[:top_k]], None
...
def cmd_show(args: argparse.Namespace) -> int:
    ...
    payload = []
    with _connect(db_path) as conn:
        for raw_key in args.source_keys:
            try:
                    ...
                    "comments": [
                        {
                            ...
                            "author": name,
                            "created_at": created,
                            "is_author": bool(post_author_id)
                            and comment_author_id == post_author_id,
                        }
                        for text, name, comment_author_id, created in comments
                    ],
                    "linked_context": [
                        {
