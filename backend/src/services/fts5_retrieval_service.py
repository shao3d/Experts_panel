...
def sanitize_fts5_query(query: str) -> str:
    ...
    if query.count("(") != query.count(")"):
        logger.warning(
            f"[FTS5 Sanitize] Unbalanced parentheses, simplifying: {original[:50]}"
        )
    ...
    if not cleaned or len(cleaned) < 2:
        logger.warning(
            f"[FTS5 Sanitize] Query empty after sanitization: {original[:50]}"
        )
        return ""
...
class FTS5RetrievalService:
    ...
    def search_posts(
        ...
        try:
            ...
            if not safe_query:
                logger.warning(
                    f"[FTS5] Query rejected after sanitization: {match_query[:50]}..."
                )
                return [], False

            # Build FTS5 query
            sql = self._build_fts5_query(
                expert_id, safe_query, cutoff_date, exclude_media_only
            )
            ...
            if len(fts5_post_ids) < self.MIN_FTS5_RESULTS:
                logger.warning(
                    f"[FTS5 Smart Fallback] Low results ({len(fts5_post_ids)} < {self.MIN_FTS5_RESULTS}), "
            ...
            if use_hybrid and self.HYBRID_SAMPLE_RATIO > 0:
                ...
                sample_size = max(
                    self.HYBRID_MIN_SAMPLE, int(total_count * self.HYBRID_SAMPLE_RATIO)
                )
                random_ids = self._get_random_post_ids(
                    expert_id, fts5_post_ids, sample_size, cutoff_date
                )

                if random_ids:
                    final_post_ids.update(random_ids)
                    logger.info(
                        f"[FTS5 Hybrid] FTS5: {len(fts5_post_ids)}, Random: {len(random_ids)}"
                    )
    ...
    def _build_fts5_query(
        ...
        """Build FTS5 SQL query without JOIN — uses expert_id/created_at from FTS5 directly.

        FTS5 table now stores expert_id and created_at as UNINDEXED columns,
        eliminating the expensive JOIN with posts table.
        """
        cutoff_condition = ""
        if cutoff_date:
            cutoff_condition = "AND f.created_at >= :cutoff_date"
    ...
    def _get_total_post_count(
        self, expert_id: str, cutoff_date: Optional[datetime] = None
    ) -> int:
