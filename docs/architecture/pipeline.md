...
- **Input**: The **pre-translation** Reduce answer plus `posts_by_id` built from `enriched_posts` (the same context Reduce saw). Verdicts are keyed by `telegram_message_id`, so they remain valid for the translated answer the user sees. For **English queries** the cited sources are first translated through the shared `TranslationService` — the same persistent cache the source panel reads — so lexical evidence fragments are substrings of exactly the translated text displayed to the user; a failed translation falls back to the original text (the LLM judge layer handles cross-language pairs).
- **Process**:
...
- **Public Query Endpoint Guards**: `POST /api/v1/query` is unauthenticated, so two in-process abuse guards run after request validation and before pipeline work (`src/services/query_rate_limit_service.py`): a per-IP sliding-window rate limit (`QUERY_RATE_LIMIT_PER_HOUR`, default 12/hour) and a global daily budget (`DAILY_QUERY_BUDGET`, default 200/UTC day). Exceeded guards return `429` with `Retry-After` (the global HTTP exception handler preserves exception headers). X-Forwarded-For is trusted only from loopback/private peers, so a direct peer cannot rotate its identity by spoofing the header. Both guards are in-process (production runs a single uvicorn worker) and reset on restart; `0` or `QUERY_RATE_LIMIT_ENABLED=false` disables them.
