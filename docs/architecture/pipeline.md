...
- **Pre-filter (FTS5 + AI Scout v3)**: If `use_super_passport` is enabled, an AI Scout generates an OR-only Entity-Centric FTS5 query to pre-filter posts. The scout is **aligned with a strict bilingual taxonomy** generated by a background cron job (Metadata Enrichment). 
- **Smart Ranking (BM25 + Time Decay)**: FTS5 results are ranked using a hybrid formula balancing lexical relevance (`f.rank` / BM25) and freshness (Time Decay with a 90-day half-life). A Random Sample (20%) is mixed in to bridge semantic gaps.
- **Service**: `MapService` (`backend/src/services/map_service.py`), `FTS5RetrievalService`, `AIScoutService`
- **Model**: `gemini-2.5-flash-lite` (Config: `MODEL_MAP`) for mapping.
- **Input**: FTS5 candidates (or all posts if Super-Passport is off/fallback triggered).
- **Chunking**: **50 posts** per chunk (`MAP_CHUNK_SIZE`).
...
| AI Scout | `MODEL_SCOUT` | `gemini-3.1-flash-lite-preview` | Entity-centric FTS5 query expansion (Taxonomy aligned). |
| Metadata | `METADATA_MODEL` | `gemini-3.1-flash-lite-preview` | Offline post metadata generation (Bilingual exact match). |
