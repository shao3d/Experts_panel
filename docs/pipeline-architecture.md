...
- **AI Scout v2**:
    - **Intent-Based Planning**: Uses `Gemini 3 Flash` to generate a search plan.
    - **Strategies**:
...
- **Deep Drill Capabilities (+500% Data)**:
    - **Scope**: Top **15** posts (was 10).
    - **Depth**: **100 comments** per post (was 50), **Depth 5** (was 3).
    - **Technique**: Proxy implements recursive fetching to find nested debates.
- **Synthesis Strategy ("Staff Engineer Persona")**:
    - **Role**: "Staff Engineer analyzing for a colleague".
    - **No Fluff**: Strict ban on generic advice.
    - **Features**:
