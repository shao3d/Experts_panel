...
I didn't build another standard chatbot. I architected this platform as an automated **"Research Laboratory"**—a highly controlled, 10-phase asynchronous pipeline (Map-Resolve-Reduce plus Reddit, Video Hub, and Meta-Synthesis sidecars) where the AI is strictly orchestrated to provide psychologically safe, zero-hallucination answers.
...
To keep API costs reasonable, the system uses a strict "Scout & Thinker" pattern. A fast, cheap model (Gemini 2.5 Flash Lite) rapidly churns through hundreds of posts to discard the noise. Only the top verified context is passed to Gemini 3-class synthesis models for final reasoning.
