...
This is the human-facing quick guide for using Panex from Codex/Claude chats
in any repo. Chat examples below are literal Russian trigger phrases — the
strings the agent actually recognizes; the query text itself can be Russian
or English.

## What Panex Does

Panex is an explicit-only bridge to production Experts Panel. It helps a parent
...
Panex should answer from its instructions and must not call the API for help
requests.
...
This prevents large Panex responses from being truncated in tool output. The
...
not create a UI-style cross-expert meta-synthesis for Panex.
...
Do not use `cat response.json` for large Panex artifacts; that can reintroduce
...
The parent chat may pass current-project context to Panex as a retrieval lens:
domain, constraints, target audience, architecture area, or decision question.
Panex should use that context only to search and deliver Experts Panel digest
...
After an `expert_digest`, Panex can reveal selected sources without rerunning
the full search:
...
response says `content_truncated=true` or `comments_truncated=true`, ask Panex
...
Default Panex answers should use `expert_digest`.
...
- Panex must be invoked explicitly.
- Panex is not default web search.
- Panex should not be called automatically for every trends/software/architecture question.
