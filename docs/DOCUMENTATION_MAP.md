...
*   **📄 `agent-context-api.md` (Accepted / AND-5..AND-16 Implemented / Forced Embeddings)**
    *   **О чём:** Спека и текущий статус agent-facing API для Codex/Claude Code: authenticated endpoint, real `source_bundle`, local CLI wrapper, BDD acceptance hardening, repo-local `experts_panel_researcher`, local dogfood/live smoke helper, paid two-expert and all-experts local smoke passed, bounded expert parallelism via `MAX_CONCURRENT_EXPERTS`, forced Embs&Keys/embedding hybrid retrieval for subagent/API calls, large all-experts-friendly budget `3600s`/`100000000` bytes, выбор экспертов, comments под `main_sources`, `external_links` как author-supplied references без auto-fetch, Signals frame, explicit-only invocation, external `--api-url` smoke mode и первый Fly.io production smoke passed for `refat,akimov`.
