...
def _recover_final_lead_report(events: list[Event]) -> str:
    """Recover a missing report.md only from the lead agent's final answer."""
    for ev in reversed(events):
        if ev.type != "message" or ev.agent_id != "lead" or ev.parent_id is not None:
            continue
        text = ev.payload.get("text")
        if not isinstance(text, str):
            continue
        report = _strip_report_saved_marker(text)
        if report:
            return report
    return ""


def _strip_report_saved_marker(text: str) -> str:
    lines = text.rstrip().splitlines()
    while lines and lines[-1].strip().startswith(REPORT_SAVED_MARKER):
        lines.pop()
