...
extract first go through a bounded repair pass for completed reports: the
adapter tries to extract the missing report URLs itself, saves successful
extracts back under `./extracts/`, and then re-runs citation verification.
URLs that still cannot be extracted are labeled `search_only_unverified`, reported in
